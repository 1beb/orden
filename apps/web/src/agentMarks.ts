// Brand marks for the agents, as inline SVG (fill="currentColor" so they inherit
// the host element's color). Shared by the sessions panel, kanban cards, and the
// project page so there's one source for the Claude / opencode glyphs.
...
// opencode square mark — official brand asset, recolored to currentColor.
...
export function markFor(agent: Agent): string {
  return agent === "opencode" ? OPENCODE_MARK : CLAUDE_MARK;
}

// A pair of clickable brand-mark buttons (Claude, opencode). Used to start an
