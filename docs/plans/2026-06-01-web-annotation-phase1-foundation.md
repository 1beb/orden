...
## Carry-forward items surfaced during Phase 1 review (address in Phase 2)

- **text-position never orphans cleanly.** `resolveSelectors` delegates text-position to `rangeFromOffsets`/`textOffsets.locate`, which *clamps* an out-of-range offset to the block's end rather than returning null. So in a `[text-quote, text-position]` fallback array, if the quote orphans and a position selector is present, resolution always lands *somewhere* — possibly mis-anchored — instead of orphaning. Phase 2's resolver/viewer work should bounds-check `start`/`end` against `block.textContent.length` (or verify the resolved text against a stored hash) before treating a position hit as a success, and add a test for ambiguous-quote + valid-position landing on the *right* range.
- **Quote scorer is duplicated** from `anchor.ts` into `selector.ts` (intentionally, since Phase 1 can't edit `anchor.ts`). The copies have already diverged by one guard (`exact.length === 0`). When `anchor.ts` is in scope for the switch-over, extract a shared `scoreOccurrences(exact, prefix, suffix, root)` and have both call it.
- **`contentHash` is string-only.** `crypto.subtle.digest` already accepts `BufferSource`; widen to `contentHash(data: string | BufferSource)` when binary sources (images/PDF) land, so they aren't hashed via a lossy string round-trip.
- **`sourceHash` is a 32-bit fnv1a key.** Collisions are tolerable *only because* the full `source` is stored alongside — the storage adapter MUST read-compare the stored source before overwriting a `<sourceHash>.json` file, never blind-write.
- **Duplicate `TextQuoteSelector`/`TextPositionSelector` names** exist in both `types.ts` (legacy, no `type` discriminant) and `wadm.ts` (new). The barrel drops the wadm ones to avoid a clash, so a Phase-2 consumer importing those names gets the *legacy* shape. Rename the wadm subtypes (or the legacy ones) so the two never share a name in the same dependency graph.
- **`migrate.ts` leans on `OrdenStatus`/`OrdenAudience` being supersets** of the legacy `AnnotationStatus`/`FeedbackTarget` unions (straight-through assignment). tsc guards this today; if the unions drift, map explicitly.
